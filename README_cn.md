...
注意：你应该使用 `except DECODE_ERRORS as e:` 捕获异常，而不是 `except msgspec.ValidationError as e:` 捕获异常。msgspecerror 可以将更多错误类型封装进 MsgspecError 对象，比如 `UnicodeDecodeError`。
...
与 `load_json_with_default` 的使用一样，但区别是没有 `utf8_error` 参数。因为 msgpack 是二进制数据，无法直接转换为 str 来处理错误的 unicode。

因此 `load_msgpack_with_default` 无法字段修复unicode 错误，一旦 UnicodeDecodeError 出现则视为跟路径错误，进而尝试默认构建跟模型，等价于 `utf8_error=='strict'`。
...
### 当字典中发生校验错误时，自动修复可能性能不佳

因为 msgspec 无法给出字典中是哪个键值发生错误，所以自动修复会遍历输入字典中的每一个键值对，尝试进行校验，直到找到错误。如果输入的字典非常大，可能性能不佳。如果你希望避免这个问题，建议全部使用 `msgspec.Struct` 定义数据结构，这样 msgspecerror 能从错误信息中精确查找。
...
### 解码msgpack时无法处理UnicodeDecodeError

因为 msgpack 是二进制数据，无法直接转换为 str 来处理错误的 unicode。
