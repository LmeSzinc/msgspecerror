...
`rename=""` 表示字段名为空字符串，路径中会有一个多余的 `.`，没有字段名跟在后面。连续多个 `rename=""` 会产生连续的点号。

当前解析器正确保留空字段名。常用组合示例：
...
**解析策略**：`_path_split_part()` 在按 `.` 切分时，保留中间和末尾的空字符串（前导空字符串是根标记的副作用，被跳过）。同时 `get_error_path()` 在去除 `$` 后的前导点时，仅在点号之后跟有实际字段名（而非 `.`、`[` 或字符串结尾）时才剥离，以保留可能的空字段开头。
